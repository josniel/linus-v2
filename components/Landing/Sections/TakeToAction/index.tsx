    ...
    >
      ...
      <div className="container relative flex flex-col items-center justify-center gap-8 max-xl:mx-auto h-[500px]">
        ...
        {/* <Image
          ...
          alt="Green"
        /> */}
        <div id="section1" className="text-white text-[48px] font-medium text-center max-lg:text-[48px] leading-normal z-[400]">
          Buy Here!
        </div>
        <div className="flex items-center max-lg:justify-center max-lg:gap-5 max-lg:flex-col lg:w-[70%] w-[100%] justify-around mt-4 z-[400]">
          {
            mainBrands.map((item, index) => (
              <a target="_blank" href={item.url} onMouseEnter={() => setAnimate(false)} onMouseLeave={() => setAnimate(true)} key={index} className={`flex items-center justify-center px-7 py-4 rounded-[17px] border-2 border-picton-blue-400 bg-picton-blue-400 bg-opacity-40 ${item.logo === '/static/images/landing/take-to-action/linex.svg' ? `w-[38%] motion-safe:animate-scale-shak` : 'w-[26%]'} max-lg:w-[80%] cursor-pointer transition-all hover:bg-opacity-100`}>
        ...
        <div className="text-white text-xs font-normal flex items-center text-center px-[30px] justify-center py-[15px] mt-2 w-[370px] max-xs:w-[80%] border-2 border-white rounded-[100px] gap-3 z-[400] cursor-pointer transform transition-all hover:bg-picton-blue-400 hover:border-picton-blue-400 " onClick={handleCopyToClipboard}>
          <Image
          ...
          {shortenString('0xe07C2bdbb8C787962C2C6e93C11a152110E7E4d2', isMobileSM ? 50:30)}
          </p>
        </div>
        <div className="flex items-center max-lg:flex-col max-lg:gap-5 lg:w-[70%] w-[100%] justify-center mt-10 z-[400] gap-1">
          {
            brands.map((item, index) => (
              <a target="_blank" href={item.url} key={index} className="flex items-center justify-center w-[30%] max-lg:w-[80%] cursor-pointer transform transition-all px-7 py-4 rounded-[17px] border-2 border-transparent hover:border-picton-blue-400 hover:bg-picton-blue-400 hover:bg-opacity-40">
                <Image src={item.logo} alt="" width={20} height={20} className="h-[40px] w-auto"/>
              </a>
            ))
          }
      ...
      <div className={`fixed bottom-5 right-5 rounded-xl border-2 border-picton-blue-400 bg-picton-blue-400 text-white font-medium text-sm flex items-center justify-center w-fit px-4 py-3 bg-opacity-80 z-[500] transition-all opacity-0 ${showTooltip ? 'opacity-100' : ''}`}>Address copied!</div>
